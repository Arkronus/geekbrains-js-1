...
let ticTakToe = {
    ...
    hasWon(){
        let size = this.config.cols;
        let line = [];
        // тут куча почти одинакового кода, но как его сделать получше яне придумал ¯\_(ツ)_/¯
        // проверяем по строчкам
        for(let i = 0; i < size; i++){
            line = [];
            for(let j = 0; j < size; j++){
                line.push(this.mapValues[i][j])
            }
            if (line.every(elem => elem === this.phase) && line.length === size){
                return true
            }
        }

        // порверяем по колонкам
        for(let i = 0; i < size; i++){
            line = [];
            for(let j = 0; j < size; j++){
                line.push(this.mapValues[j][i]);
            }
            if (line.every(elem => elem === this.phase) && line.length === size){
                return true;
            }
        ...
        for(let i = 0; i < size; i++){
            let colNum = size-1-i;
            line.push(this.mapValues[i][colNum])
            if (line.every(elem => elem === this.phase) && line.length === size){
                return true
            }
